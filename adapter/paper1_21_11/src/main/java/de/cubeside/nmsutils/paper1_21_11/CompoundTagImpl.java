...
public final class CompoundTagImpl implements de.cubeside.nmsutils.nbt.CompoundTag {
    ...
    public boolean containsKey(String name, TagType type) {
        Tag value = handle.get(name);
        return value != null && value.getType() == TagTypes.getType(type.internalId());
    }
    ...
    public Set<String> getAllKeys() {
        return Collections.unmodifiableSet(handle.keySet());
    }
    ...
    public UUID getUUID(String name) {
        return handle.read("UUID", UUIDUtil.CODEC).orElse(null);
    }

    @Override
    public boolean containsUUID(String name) {
        return handle.read("UUID", UUIDUtil.CODEC).isPresent();
    }
